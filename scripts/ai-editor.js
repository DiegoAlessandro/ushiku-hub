...
【タスク2: 属性タグの抽出（超重要）】
以下の定型タグリストから、内容に該当するものを優先的に選んでください。
定型タグ: 牛久駅, ひたち野うしく駅, 牛久市中心部, 駐車場あり, 駅近, テイクアウト可, 子連れ歓迎, 家族向け, ランチ, モーニング, ディナー, 新規オープン, 期間限定, 無料, アルバイト募集, パート募集
ルール:
- 該当する定型タグを必ずそのままの文字列で tags 配列に含めること（表記揺れ禁止）
- 定型タグに該当しない独自の属性がある場合のみ、追加タグとして自由に付けてよい
- タグは合計3〜5個を目安にすること
